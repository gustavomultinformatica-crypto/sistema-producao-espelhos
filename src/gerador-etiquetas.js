...
async function imprimirEtiquetas(lista,modelo){
  ...
  *{box-sizing:border-box}
  ...
  .label{position:absolute;left:3mm;top:3mm;width:54mm;height:31mm;display:grid;grid-template-rows:6mm 18mm 7mm;align-items:center;justify-items:center;overflow:hidden}
  .modelo{width:52mm;text-align:center;font-size:10pt;font-weight:800;line-height:6mm;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .barcode{width:52mm;height:18mm;display:flex;align-items:center;justify-content:center;overflow:hidden}
  .barcode svg{display:block!important;max-width:50mm!important;width:auto!important;height:16mm!important;margin:0 auto!important}
  .cod{width:52mm;text-align:center;font-size:9pt;font-weight:800;line-height:6mm;white-space:nowrap;overflow:hidden;padding-bottom:1mm}
  ...
  codigos.forEach((c,i)=>JsBarcode('#b'+i,c,{format:'CODE128',displayValue:false,height:50,width:1.55,margin:0,background:'#fff',lineColor:'#000'}));
...
async function abrirGerador(scanner){
  ...
  const card=document.createElement('div');Object.assign(card.style,{background:'#fff',borderRadius:'18px',padding:'22px',width:'min(96vw,760px)',boxShadow:'0 20px 60px rgba(0,0,0,.25)'});
  card.innerHTML=`<h2 style="margin:0 0 6px">Gerar etiquetas 6x4 cm</h2><p style="margin:0 0 14px;color:#64748b">Formato definitivo <b>60 x 40 mm</b> com margem de segurança maior para evitar corte na Zebra ZD220.</p><div style="display:grid;grid-template-columns:minmax(0,1fr) 260px;gap:18px;align-items:start"><div><label style="display:block;font-weight:700;margin-bottom:12px">Modelo<select class="lgModelo" style="width:100%;margin-top:6px;padding:11px;border:1px solid #cbd5e1;border-radius:10px"></select></label><label style="display:block;font-weight:700;margin-bottom:16px">Quantidade<input class="lgQtd" type="number" min="1" max="300" value="1" style="width:100%;margin-top:6px;padding:11px;border:1px solid #cbd5e1;border-radius:10px"></label><div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:12px;margin-bottom:14px"><b>Zebra ZD220</b><br><span style="font-size:13px;color:#475569">Papel 60 x 40 mm · escala 100% · não usar “Ajustar à página”.</span></div><div class="lgMsg" style="min-height:20px;color:#475569;margin-bottom:12px"></div><div style="display:flex;gap:8px;justify-content:flex-end"><button class="lgCancelar" type="button" style="padding:10px 14px;border-radius:10px;border:1px solid #cbd5e1;background:#fff;font-weight:700">Cancelar</button><button class="lgGerar" type="button" style="padding:10px 14px;border-radius:10px;border:0;background:#2563eb;color:#fff;font-weight:800">GERAR ETIQUETA 6x4</button></div></div><div><b style="display:block;margin-bottom:8px">Prévia</b><div style="width:240px;height:160px;background:#fff;border:1px solid #cbd5e1;border-radius:14px;padding:16px;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:0 8px 24px rgba(15,23,42,.08)"><div class="prevModelo" style="font-size:18px;font-weight:800;margin-bottom:10px">${modeloPadrao||'Modelo'}</div><div style="width:190px;height:52px;background:repeating-linear-gradient(90deg,#111 0 2px,#fff 2px 4px,#111 4px 7px,#fff 7px 10px)"></div><div style="font-size:17px;font-weight:800;margin-top:10px">CÓDIGO DA PEÇA</div></div></div></div>`;
  modal.appendChild(card);document.body.appendChild(modal);const sel=card.querySelector('.lgModelo');const opcoes=listaModelos.length?listaModelos:[modeloPadrao].filter(Boolean);opcoes.forEach(n=>{const o=document.createElement('option');o.value=n;o.textContent=n;sel.appendChild(o)});if(modeloPadrao&&opcoes.includes(modeloPadrao))sel.value=modeloPadrao;const prev=card.querySelector('.prevModelo');sel.addEventListener('change',()=>{prev.textContent=sel.value||'Modelo'});
  card.querySelector('.lgCancelar').onclick=fecharModal;card.querySelector('.lgGerar').onclick=async()=>{const qtd=Math.max(1,Math.min(300,Number(card.querySelector('.lgQtd').value||1)));const modelo=sel.value,msg=card.querySelector('.lgMsg'),btn=card.querySelector('.lgGerar');if(!modelo)return msg.textContent='Selecione um modelo.';btn.disabled=true;btn.textContent='Gerando...';msg.textContent='Reservando códigos...';const codigos=Array.from({length:qtd},(_,i)=>codigoPeca(i+1));const {error}=await supabase.from('produtos').insert(codigos.map(c=>({codigo_barras:c,modelo,ativo:true})));if(error){btn.disabled=false;btn.textContent='GERAR ETIQUETA 6x4';msg.textContent='Erro: '+error.message;return;}msg.textContent=`${qtd} etiqueta(s) gerada(s).`;await imprimirEtiquetas(codigos,modelo);setTimeout(fecharModal,700);};
}
