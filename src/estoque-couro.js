...
function estilos(){
 ...
 .ecFilters{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:12px}.ecFilters select,.ecFilters input{width:auto;min-width:160px;margin:0;padding:10px}.ecList{display:flex;flex-direction:column}.ecRow{display:grid;grid-template-columns:110px 1fr 100px 120px 1fr 140px 150px;gap:10px;align-items:center;padding:11px 8px;border-bottom:1px solid #edf2f7;font-size:13px}.ecRow small{display:block;color:#64748b;margin-top:3px}.ecBadge{display:inline-flex;width:max-content;border-radius:999px;padding:5px 8px;font-size:11px;font-weight:900}.ecBadge.e{background:#dcfce7;color:#166534}.ecBadge.s{background:#fee2e2;color:#991b1b}.ecEmpty{text-align:center;padding:28px;color:#94a3b8}.ecActions{display:flex;gap:6px;justify-content:flex-end}.ecActions button{width:auto;margin:0;padding:8px 10px;font-size:11px}.ecEdit{background:#eff6ff;color:#1d4ed8}.ecDelete{background:#fee2e2;color:#991b1b}.ecEditing{background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:10px 12px;color:#9a3412;font-weight:800;font-size:13px;margin-bottom:12px;display:none}
 @media(max-width:1100px){.ecGrid{grid-template-columns:1fr}.ecStockGrid,.ecCards{grid-template-columns:1fr 1fr}.ecRow{grid-template-columns:1fr 1fr}.ecRow time,.ecActions{grid-column:1/-1}.ecActions{justify-content:flex-start}.ecTop{align-items:flex-start;flex-direction:column}.ecBack{width:100%}}
 @media(max-width:600px){.ecOverlay{padding:10px}.ecForm,.ecStockGrid,.ecCards{grid-template-columns:1fr}.ecFull,.ecSubmit,.ecCancel{grid-column:auto}.ecRow{grid-template-columns:1fr}.ecFilters select,.ecFilters input{width:100%}}
...
function render(){
 ...
 const box=o.querySelector('[data-ec-list]');if(!lista.length){box.innerHTML='<div class="ecEmpty">Nenhuma movimentação encontrada.</div>';return}
 box.innerHTML=lista.slice(0,500).map(m=>`<div class="ecRow"><div><span class="ecBadge ${m.tipo==='ENTRADA'?'e':'s'}">${m.tipo==='ENTRADA'?'Entrada':'Saída'}</span><small>${fmtData(m.data_movimentacao)}</small></div><div><b>${esc(m.cor)}</b><small>${esc(m.fornecedor||'Sem fornecedor')}</small></div><strong>${fmtNum(m.quantidade_metros)} m</strong><div><b>${esc(m.documento||'-')}</b><small>documento</small></div><div><small>${esc(m.observacao||'Sem observação')}</small><small>por ${esc(nomeUsuario(m.usuario_id))}</small></div><time>${new Date(m.criado_em).toLocaleString('pt-BR',{dateStyle:'short',timeStyle:'short'})}</time><div class="ecActions"><button type="button" class="ecEdit" data-ec-edit="${m.id}">ALTERAR</button><button type="button" class="ecDelete" data-ec-delete="${m.id}">EXCLUIR</button></div></div>`).join('');
 box.querySelectorAll('[data-ec-edit]').forEach(b=>b.addEventListener('click',()=>iniciarEdicao(b.dataset.ecEdit)));
 box.querySelectorAll('[data-ec-delete]').forEach(b=>b.addEventListener('click',()=>excluirMovimento(b.dataset.ecDelete)));
}

function limparFormulario(){
 const o=document.querySelector('.ecOverlay');if(!o)return;const hoje=new Date(),data=`${hoje.getFullYear()}-${String(hoje.getMonth()+1).padStart(2,'0')}-${String(hoje.getDate()).padStart(2,'0')}`;
 editandoId=null;o.querySelector('[data-ec-tipo]').value='ENTRADA';o.querySelector('[data-ec-data]').value=data;o.querySelector('[data-ec-cor]').value='';o.querySelector('[data-ec-qtd]').value='';o.querySelector('[data-ec-fornecedor]').value='';o.querySelector('[data-ec-doc]').value='';o.querySelector('[data-ec-obs]').value='';o.querySelector('[data-ec-submit]').textContent='SALVAR MOVIMENTAÇÃO';o.querySelector('[data-ec-cancel]').style.display='none';o.querySelector('[data-ec-editing]').style.display='none';
}

function iniciarEdicao(id){
 const o=document.querySelector('.ecOverlay');if(!o)return;const m=movimentos.find(x=>x.id===id);if(!m)return;editandoId=id;o.querySelector('[data-ec-tipo]').value=m.tipo;o.querySelector('[data-ec-data]').value=String(m.data_movimentacao).slice(0,10);o.querySelector('[data-ec-cor]').value=m.cor||'';o.querySelector('[data-ec-qtd]').value=m.quantidade_metros;o.querySelector('[data-ec-fornecedor]').value=m.fornecedor||'';o.querySelector('[data-ec-doc]').value=m.documento||'';o.querySelector('[data-ec-obs]').value=m.observacao||'';o.querySelector('[data-ec-submit]').textContent='SALVAR ALTERAÇÕES';o.querySelector('[data-ec-cancel]').style.display='block';const aviso=o.querySelector('[data-ec-editing]');aviso.style.display='block';aviso.textContent=`Editando ${m.tipo==='ENTRADA'?'entrada':'saída'} de ${fmtNum(m.quantidade_metros)} m de couro ${m.cor}.`;o.querySelector('[data-ec-cor]').focus();o.scrollTo({top:0,behavior:'smooth'});
}

async function excluirMovimento(id){
 const m=movimentos.find(x=>x.id===id);if(!m)return;const ok=window.confirm(`Excluir ${m.tipo==='ENTRADA'?'a entrada':'a saída'} de ${fmtNum(m.quantidade_metros)} m de couro ${m.cor}?\n\nEssa ação altera o saldo do estoque.`);if(!ok)return;
 const o=document.querySelector('.ecOverlay'),msg=o?.querySelector('[data-ec-msg]');
 try{const{error}=await supabase.from('estoque_couro_movimentacoes').delete().eq('id',id);if(error)throw error;if(editandoId===id)limparFormulario();if(msg){msg.className='ecMsg ok';msg.textContent='✓ Movimentação excluída. O saldo do estoque foi recalculado.'}await carregar()}catch(err){if(msg){msg.className='ecMsg err';msg.textContent=err.message||'Erro ao excluir movimentação.'}}
}
...
async function registrar(e){
 ...
 btn.disabled=true;btn.textContent=editandoId?'SALVANDO ALTERAÇÕES...':'SALVANDO...';msg.className='';msg.textContent='';
 try{
  const{data:{session}}=await supabase.auth.getSession();if(!session)throw new Error('Sessão expirada. Entre novamente.');
  const payload={cor,tipo,quantidade_metros:qtd,data_movimentacao:data,fornecedor:fornecedor||null,documento:documento||null,observacao:observacao||null,usuario_id:session.user.id};
  if(editandoId){const{error}=await supabase.from('estoque_couro_movimentacoes').update(payload).eq('id',editandoId);if(error)throw error;msg.className='ecMsg ok';msg.textContent=`✓ Movimentação alterada para ${fmtNum(qtd)} m de couro ${cor}.`;limparFormulario()}else{const{error}=await supabase.from('estoque_couro_movimentacoes').insert(payload);if(error)throw error;msg.className='ecMsg ok';msg.textContent=`✓ ${tipo==='ENTRADA'?'Entrada':'Saída'} de ${fmtNum(qtd)} m de couro ${cor} registrada.`;o.querySelector('[data-ec-qtd]').value='';o.querySelector('[data-ec-doc]').value='';o.querySelector('[data-ec-obs]').value=''}
  await carregar();
 }catch(err){msg.className='ecMsg err';msg.textContent=err.code==='42P01'?'A tabela de estoque de couro ainda não foi ativada no Supabase.':(err.message||'Erro ao salvar movimentação.')}finally{btn.disabled=false;btn.textContent=editandoId?'SALVAR ALTERAÇÕES':'SALVAR MOVIMENTAÇÃO'}
}

async function abrir(){
 estilos();document.querySelector('.ecOverlay')?.remove();editandoId=null;const hoje=new Date(),data=`${hoje.getFullYear()}-${String(hoje.getMonth()+1).padStart(2,'0')}-${String(hoje.getDate()).padStart(2,'0')}`;const o=document.createElement('section');o.className='ecOverlay';o.innerHTML=`<div class="ecShell"><div class="ecTop"><div><small>CONTROLE DE ESTOQUE</small><h1>Estoque de couro</h1><p>Registre a metragem recebida por cor e acompanhe o saldo de cada couro.</p></div><button type="button" class="ecBack">← Voltar</button></div><div class="ecGrid"><div class="ecPanel"><h2>Nova movimentação</h2><div class="ecSub">Para chegada do fornecedor, deixe o tipo como Entrada. Use Saída quando quiser registrar consumo.</div><div class="ecEditing" data-ec-editing></div><form class="ecForm" data-ec-form><label>Tipo<select data-ec-tipo><option value="ENTRADA">Entrada / recebimento</option><option value="SAIDA">Saída / consumo</option></select></label><label>Data<input type="date" data-ec-data value="${data}"></label><label>Cor<input list="ec-cores" data-ec-cor placeholder="Ex.: Preto"><datalist id="ec-cores">${CORES_PADRAO.map(c=>`<option value="${c}">`).join('')}</datalist></label><label>Quantidade em metros<input type="number" step="0.01" min="0.01" data-ec-qtd placeholder="Ex.: 60"></label><label>Fornecedor<input data-ec-fornecedor placeholder="Nome do fornecedor"></label><label>Nota / documento<input data-ec-doc placeholder="Ex.: NF 12345"></label><label class="ecFull">Observação<textarea data-ec-obs placeholder="Ex.: lote com tonalidade diferente, bobina 3..."></textarea></label><button class="ecSubmit" data-ec-submit>SALVAR MOVIMENTAÇÃO</button><button type="button" class="ecCancel" data-ec-cancel>CANCELAR ALTERAÇÃO</button></form><div data-ec-msg></div></div><div class="ecPanel"><div class="ecCards"><div class="ecCard"><span>Estoque atual</span><strong data-ec-total>0 m</strong></div><div class="ecCard"><span>Total recebido</span><strong data-ec-entradas>0 m</strong></div><div class="ecCard"><span>Total consumido</span><strong data-ec-saidas>0 m</strong></div></div><h2>Saldo por cor</h2><div class="ecSub">Preto, Café, Caramelo, Gelo, Rosa e qualquer nova cor cadastrada.</div><div class="ecStockGrid" data-ec-stock><div class="ecEmpty">Carregando...</div></div></div></div><div class="ecPanel"><h2>Histórico de movimentações</h2><div class="ecSub">Use ALTERAR para corrigir um lançamento ou EXCLUIR para remover uma movimentação errada.</div><div class="ecFilters"><select data-ec-filter-cor><option value="todas">Todas as cores</option></select><select data-ec-filter-tipo><option value="todos">Entradas e saídas</option><option value="ENTRADA">Somente entradas</option><option value="SAIDA">Somente saídas</option></select></div><div class="ecList" data-ec-list><div class="ecEmpty">Carregando...</div></div></div></div>`;document.body.appendChild(o);o.querySelector('.ecBack').onclick=()=>o.remove();o.querySelector('[data-ec-form]').addEventListener('submit',registrar);o.querySelector('[data-ec-cancel]').addEventListener('click',()=>{limparFormulario();const msg=o.querySelector('[data-ec-msg]');msg.className='';msg.textContent=''});o.querySelector('[data-ec-filter-cor]').addEventListener('change',render);o.querySelector('[data-ec-filter-tipo]').addEventListener('change',render);try{await carregar()}catch(err){o.querySelector('[data-ec-list]').innerHTML=`<div class="ecEmpty">Não foi possível carregar o estoque: ${esc(err.message||'erro')}</div>`}}
