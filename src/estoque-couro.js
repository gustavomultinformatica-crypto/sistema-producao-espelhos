...
async function carregar(){
 const [{data:mov,error:em},{data:ps,error:ep}]=await Promise.all([
  supabase.from('estoque_couro_movimentos').select('id,cor,tipo,quantidade_metros,data_movimento,fornecedor,documento,observacao,usuario_id,criado_em').order('data_movimento',{ascending:false}).order('criado_em',{ascending:false}).limit(2000),
...
function render(){
 ...
 const box=o.querySelector('[data-ec-list]');if(!lista.length){box.innerHTML='<div class="ecEmpty">Nenhuma movimentação encontrada.</div>';return}box.innerHTML=lista.slice(0,500).map(m=>`<div class="ecRow"><div><span class="ecBadge ${m.tipo==='entrada'?'e':'s'}">${m.tipo==='entrada'?'Entrada':'Saída'}</span><small>${fmtData(m.data_movimento)}</small></div><div><b>${esc(m.cor)}</b><small>${esc(m.fornecedor||'Sem fornecedor')}</small></div><strong>${fmtNum(m.quantidade_metros)} m</strong><div><b>${esc(m.documento||'-')}</b><small>documento</small></div><div><small>${esc(m.observacao||'Sem observação')}</small><small>por ${esc(nomeUsuario(m.usuario_id))}</small></div><time>${new Date(m.criado_em).toLocaleString('pt-BR',{dateStyle:'short',timeStyle:'short'})}</time></div>`).join('');
}
