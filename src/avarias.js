...
function estilos(){
 ...
 .avNav{display:flex!important;align-items:center;gap:7px;white-space:nowrap;background:#fff7ed!important;color:#9a3412!important;border-color:#fed7aa!important}
 ...
 .avGrid{display:grid;grid-template-columns:.85fr 1.15fr;gap:16px}.avPanel{padding:20px}.avPanel h2{margin:0 0 4px}.avSub{color:#64748b;font-size:13px;margin-bottom:16px}.avHint{background:#fff7ed;border:1px solid #fed7aa;color:#9a3412;border-radius:12px;padding:12px 14px;font-size:13px;font-weight:700;margin-bottom:14px}
...
function tipoLabel(tipo){return tipo==='Quebrada'?'Peça quebrada':'Peça danificada'}
...
function renderLista(){
 ...
 if(!registros.length){lista.innerHTML='<div class="avEmpty">Nenhuma ocorrência registrada.</div>';return}
 lista.innerHTML=registros.slice(0,100).map(r=>`<div class="avRow"><div><b>${esc(r.produtos?.codigo_barras||'-')}</b><small>${esc(r.produtos?.modelo||'-')}</small></div><div><span class="avBadge ${r.tipo==='Quebrada'?'q':'a'}">${esc(tipoLabel(r.tipo))}</span><small>${esc(r.motivo||'Sem motivo informado')}</small></div><div><b>${esc(SETORES[Number(r.setor_id)]||'-')}</b><small>setor da ocorrência</small></div><div><small>${esc(r.observacao||'Sem observação')}</small></div><time>${fmt(r.criado_em)}</time></div>`).join('');
}
...
async function registrar(e){
 ...
 try{
  ...
  const {data:produto,error:ep}=await supabase.from('produtos').select('id,codigo_barras,modelo').eq('codigo_barras',codigo).maybeSingle();if(ep)throw ep;if(!produto)throw new Error('Etiqueta não encontrada. Essa peça precisa existir na produção antes de registrar a ocorrência.');
  ...
  if(error){if(error.code==='42P01')throw new Error('A estrutura de ocorrências ainda não foi ativada no banco.');throw error}
  msg.className='avMsg ok';msg.textContent=`✓ Ocorrência registrada: ${produto.modelo} (${codigo}) — ${tipoLabel(tipo)}.`;
  o.querySelector('[data-av-codigo]').value='';o.querySelector('[data-av-obs]').value='';o.querySelector('[data-av-motivo]').value='';o.querySelector('[data-av-codigo]').focus();await carregar();
 }catch(err){msg.className='avMsg err';msg.textContent=err.message||'Erro ao registrar ocorrência.'}
 finally{bot.disabled=false;bot.textContent='REGISTRAR OCORRÊNCIA'}
}

async function abrir(){
 estilos();document.querySelector('.avOverlay')?.remove();const o=document.createElement('section');o.className='avOverlay';
 o.innerHTML=`<div class="avShell"><div class="avTop"><div><small>CONTROLE DE OCORRÊNCIAS</small><h1>Peças quebradas ou danificadas</h1><p>Tela separada da bipagem normal. Use a mesma etiqueta da peça para registrar o problema.</p></div><button type="button" class="avBack">← Voltar</button></div><div class="avGrid"><div class="avPanel"><h2>Registrar ocorrência</h2><div class="avSub">Bipe a etiqueta com o leitor de código de barras ou digite o código manualmente.</div><div class="avHint">Este registro não substitui nem altera a bipagem normal da produção. Ele cria um histórico separado da ocorrência.</div><form class="avForm" data-av-form><label>Código da etiqueta<input data-av-codigo autofocus autocomplete="off" placeholder="Bipe a mesma etiqueta da produção..."></label><label>Tipo de ocorrência<select data-av-tipo><option value="Quebrada">Peça quebrada</option><option value="Avariada">Peça danificada / avariada</option></select></label><label>Setor onde aconteceu<select data-av-setor>${Object.entries(SETORES).map(([id,n])=>`<option value="${id}">${id}. ${n}</option>`).join('')}</select></label><label>Motivo<input data-av-motivo placeholder="Ex.: caiu no chão, trincou, riscou, couro danificado..."></label><label>Observação<textarea data-av-obs placeholder="Detalhes adicionais sobre a ocorrência (opcional)"></textarea></label><button class="avSubmit" data-av-submit>REGISTRAR OCORRÊNCIA</button></form><div data-av-msg></div></div><div class="avPanel"><div class="avCards"><div class="avCard"><span>Ocorrências hoje</span><strong data-av-total>0</strong></div><div class="avCard"><span>Quebradas hoje</span><strong data-av-quebradas>0</strong></div><div class="avCard"><span>Danificadas hoje</span><strong data-av-avariadas>0</strong></div></div><h2>Últimas ocorrências</h2><div class="avSub">Histórico recente das peças quebradas ou danificadas.</div><div class="avList" data-av-list><div class="avEmpty">Carregando...</div></div></div></div></div>`;
 ...
 try{await carregar()}catch(err){const lista=o.querySelector('[data-av-list]');lista.innerHTML=`<div class="avEmpty">Não foi possível carregar as ocorrências: ${esc(err.message||'erro')}</div>`}
}
